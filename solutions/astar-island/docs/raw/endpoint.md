...
Source: challenge://astar-island/endpoint
...
| Field | Type | Description |
|-------|------|-------------|
| `round_score` | float \| null | Average score across all seeds (null if not scored) |
...
| Field | Type | Description |
|-------|------|-------------|
| `seed_index` | int | Which seed (0–4) |
| `argmax_grid` | int[][] | H×W grid of predicted class indices |
| `confidence_grid` | float[][] | H×W grid of max probability per cell (3 decimal places) |
...
Post-round ground truth comparison. Only available after round is completed or during scoring.
