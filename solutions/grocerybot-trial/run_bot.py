...
class TrialBot:
    ...
    def _build_greedy_assignments(
        ...
        locked_by_others = Counter()
        for item_id in self.bot_targets.values():
            for item in items:
                if item["id"] == item_id:
                    locked_by_others[item["type"]] += 1
                    break
        for item_type, count in locked_by_others.items():
            if needed_left[item_type] > 0:
                needed_left[item_type] = max(0, needed_left[item_type] - count)
