...
ENDPOINT_CATALOG: list[dict[str, Any]] = [
    ...
    },
    {
        "method": "POST",
        "path": "/project",
        "required_fields": ["name", "number", "projectManagerId"],
        "optional_fields": [
    ...
    },
    {
        "method": "PUT",
        "path": "/project/{id}",
        "required_fields": ["id", "name", "number", "projectManagerId"],
        "optional_fields": [
    ...
    },
    {
        ...
        "required_fields": ["invoiceDate", "invoiceDueDate", "customer", "orders"],
        "optional_fields": ["comment", "paymentTypeId"],
        "description": (
            "Create an invoice. Requires an order with orderlines. "
            "Pass ?sendToCustomer=true to send. The 'customer' field should be "
            "an object like {\"id\": <customer_id>}. The 'orders' field should be a "
            "list of order objects with 'orderLines'."
        ),
    },
    {
        "method": "GET",
    ...
    },
    {
        ...
        "optional_fields": ["orderLines", "receiver"],
        "description": (
            "Create an order. The 'customer' field should be {\"id\": <customer_id>}. "
            "Each order line needs 'product' (with id), 'count', and optional price override."
        ),
    },
    {
        "method": "POST",
